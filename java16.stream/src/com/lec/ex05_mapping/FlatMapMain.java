...
/*
	...
	매핑은 중간처리기능으로 스트림의 요소를 다른 요소로 대체하는 작업.
	스트림에서 제공하는 매핑메서드는 flatMapXXX(), mapXXX(), asXXXStream(), boxed()가 있다.
	
	1. flatMapXXX()  : 요소를 대체하는 복수개의 요소들로 구성된 새로운 스트림을 리턴
	2. mapXXX()		 : 요소를 대체하는 요소로 구성된 새로운 스트림을 리턴
	3. asXXXStream()
		a. asDoubleStream() : IntStream의 int, LongStream의 long의 값들을 double타입으로 리턴
		b. asLongStream() 	: IntStream의 int값을 long으로 타입변환 후 LongStream으로 리턴
	4. boxed()		 : int, long, double 기본타입을 각각 Integer, Long, Double로 박싱해서 stream으로 리턴
 */

public class FlatMapMain {

	public static void main(String[] args) {
		// 1. flatMap : 복수개의 요소로 대체
