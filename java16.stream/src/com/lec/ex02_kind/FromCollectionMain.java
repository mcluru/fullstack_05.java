...
/*
	...
	java8부터 새로 추가된 java.util.stream패키지는 stream API들이 포함되어 있다.
	stream의 종류는 BaseStream과 이 스트림을 구현한 Stream, IntStream, LongStream, DoubleStream이 있다.
	
	BaseStream은 모든 스트림에서 사용할 수 있는 공통메서드가 정의되어 있을 뿐, 코드에선 직접 사용 안한다.
	하위 스트림이 직접적으로 사용되는 스트림이다. Stream은 객체의 요소를 처리하는 스트림이고
