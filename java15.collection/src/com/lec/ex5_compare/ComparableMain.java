...
/*
	...
	타입은 값을 정렬하고, 문자열(String)타입은 유니코드로 정렬된다
	
	TreeSet객체와 TreeMap을 정렬하기 위해선 java.lang.Comparable인터페이스를 구현한 객체를 요구하는데
	Integer, Double, String등으로 모두 Comparable인터페이스를 구현하고 있다
	
	사용자가 정의한 클래스를 TreeSet이나 TreeMap에 저장할 경우에 Comparable을 구현하고 있지 않으면
	자동정렬을 할 수 없다. Comparable인터페이스는 compareTo() 추상메서드가 정의되어 있으므로 사용자가
	정의하는 클래스도 Comparable을 구현하려면 CompareTo()메서드를 재정의해야한다.
	
	compareTo()메서드는 int값을 리턴한다. 객체가 동일하면 0, 작으면 -1, 크면 1을 리턴한다
	TreeSet객체와 TreeMap의 키가 Comparable을 구현하지 않으면 객체를 저장하는 순간 Class CastException예외가 발생한다
 */
