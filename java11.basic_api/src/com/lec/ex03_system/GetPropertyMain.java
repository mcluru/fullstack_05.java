package com.lec.ex03_system;

import java.util.*;

/*
	System 클래스
	
	5. 시스템프로퍼티 읽기(getProperty(), getProperties())
		시스템프로퍼티는 JVM이 시작될때 자동으로 설정되는 시스템의 속성값들을 의미.
		예를들어 운영체제의 종료, 자바프로그램을 실행시킨 사용자ID, JVM버전, 운영체제에서
		사용되는 파일경로, 구분자 등이다
		시스템프로퍼티는 key와 value로 구성되어있다.
*/

public class GetPropertyMain {

	public static void main(String[] args) {
		
		String os_name = System.getProperty("os.name");
		String user_name = System.getProperty("user.name");
		String user_home = System.getProperty("user.home");
		
		System.out.println("운영체제의 이름 = " + os_name);
		System.out.println("사용자 이름 = " + user_name);
		System.out.println("사용자 홈디렉토리 = " + user_home);
		System.out.println();
		
		System.out.println("---------------------------------");
		System.out.println("      [Key]          [Value]     ");
		System.out.println("---------------------------------");
		
		Properties props = System.getProperties();
		Set keys = props.keySet();
		for(Object key:keys) {
			String keyVal = (String) key;
			String value = System.getProperty(keyVal);
			System.out.println(" [ " + keyVal + " ] " + value);
		}
		
	}

}
